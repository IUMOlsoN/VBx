...
def AHC_neighbor(pair_similarity, threshold):
    ...
    while True:
        ...
        # need to find neighbor clusters ids
        minus = find_neighbor_clusters(clsts,mi-1) #TODO: account for mi=0 case
        base  = find_neighbor_clusters(clsts,mi)
        plus = find_neighbor_clusters(clsts,mi+1, backward=False) #TOD0: account for mi+1=len(clsts)
        # TODO: check for repeats ([minus,base],[base,plus])
        if base < len(dist) - 1:
            #TODO: fix indics; dist indices are different from cluster indices
            #  should be no infinity dist value in the update
            #TODO: check the distance update calculation; should cosine similarity be re-calculated?
            dist[plus] = (dist[base]*len(clsts[base]) + dist[plus]*len(clsts[plus]))/(len(clsts[base])+len(clsts[plus]))
        if base != 0: 
            dist[minus] = (dist[base]*len(clsts[base]) + dist[minus]*len(clsts[minus]))/(len(clsts[base])+len(clsts[minus]))
        # set merged pair distance to infinity
        dist[mi] = np.inf
        # update clusters
        clsts[base].extend(clsts[plus])
        clsts[plus] = None
