...
def AHC_neighbor(pair_similarity, threshold):
    ...
    while True:
        ...
        # Find the indices of the clusters to update
        # base is cluster corrseponding to mi
        base  = find_neighbor_clusters(clsts,mi)
        # minus is id for mi-1
        minus = find_neighbor_clusters(clsts,base-1)
        # plus is id for mi+1
        plus = find_neighbor_clusters(clsts,mi+1, backward=False) #TOD0: account for mi+1=len(clsts)

        if base < len(dist) - 1 and clsts[plus][-1] < len(dist):    
            dist[clsts[plus][-1]] = (dist[mi]*len(clsts[base]) + dist[clsts[plus][-1]]*len(clsts[plus]))/(len(clsts[base])+len(clsts[plus]))
        if base != 0 and clsts[minus][-1] >= 0: 
            dist[clsts[minus][-1]] = (dist[mi]*len(clsts[base]) + dist[clsts[minus][-1]]*len(clsts[minus]))/(len(clsts[base])+len(clsts[minus]))
