    ...
    for file_name, segs in recit:
        ...
        if args.init == 'AHC' or args.init.endswith('VB'):
            if args.init.startswith('AHC'):
                ...
                start = time.time()
                dist_vector = pdist(x,metric='cosine')
                dist_mx = squareform(dist_vector)
                # Figure out utterance specific args.threshold for AHC.
                thr, junk = twoGMMcalib_lin(dist_mx.ravel())
                # output "labels" is an integer vector of speaker (cluster) ids
                x_linkage = linkage(dist_vector, method='average')
                labels1st = fcluster(x_linkage, thr - args.threshold, criterion='distance')
